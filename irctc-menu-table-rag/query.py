"""
IRCTC South Central Menu RAG Query Assistant.
...
def interactive_mode(embedder, reranker, client) -> None:
    ...
    console.print(Panel(body, title="IRCTC South Central Menu - RAG Assistant", border_style="blue", expand=False))
...
def main() -> None:
    parser = argparse.ArgumentParser(description="IRCTC South Central Menu RAG Query")
    ...
    if not Path(QDRANT_PATH).exists():
        console.print(
            ...
            "Run [bold]python build_index.py[/bold] first."
        )
        sys.exit(1)
